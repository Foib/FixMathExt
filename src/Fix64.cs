...
public partial struct Fix64 : IEquatable<Fix64>, IComparable<Fix64>
{
    ...
    private static readonly Fix64 LutInterval = (LUT_SIZE - 1) / PiOver2;
    ...
    ///     Returns Fix64.One if the value is positive, Fix64.Zero if is 0, and -Fix64.One if it is negative.
    /// </summary>
    public static Fix64 Sign(Fix64 value)
    {
        return
            value.RawValue < 0 ? -One :
            value.RawValue > 0 ? One :
            Zero;
    }
    ...
    /// </summary>
    public static int SignInt(Fix64 value)
    {
        return
            value.RawValue < 0 ? -1 :
            value.RawValue > 0 ? 1 :
    ...
    {
        if (x.RawValue <= 0) throw new ArgumentOutOfRangeException("Non-positive value passed to Ln", "x");
    ...
    public static Fix64 Sin(Fix64 x)
    {
        ...
        var secondNearestValue =
            new Fix64(SinLut[
                flipHorizontal
                    ? SinLut.Length - 1 - (int)roundedIndex - SignInt(indexError)
                    : (int)roundedIndex + SignInt(indexError)]);
    ...
    public static Fix64 Tan(Fix64 x)
    {
        ...
        var secondNearestValue = new Fix64(TanLut[(int)roundedIndex + SignInt(indexError)]);
    ...
    public static Fix64 Remainder(Fix64 x, Fix64 y)
    {
        ...
        var num = d - Abs(y) * Sign(x);
        if (Abs(num) == Abs(d))
        {
            var a = x / y;
            return Abs(Round(a)) > Abs(a) ? num : d;
        }

        return Abs(num) < Abs(d) ? num : d;
    }
